  ...
  try {
    ...
    if (Array.isArray(planDefinition.library) || typeof(planDefinition.library) === 'string') {
      ...
      var patientBundle = {
        ...
        id: 'survey-bundle',
        type: 'collection',
        entry: (await resolver('Patient/')).map(r => {return {resource: r}})
      };
    ...
    if (applyThisCondition) {
      ...
      if (def) {
        ...
        if (/PlanDefinition/.test(def)) {
          ...
          applied.resource = { reference: 'RequestGroup/' + RequestGroup.id };

          // Set the status of the target RequestGroup to draft
          RequestGroup.status = 'draft';
          ...
          if (act?.dynamicValue) {
            ...
            RequestGroup = evaluatedValues.reduce((acc, cv) => {
